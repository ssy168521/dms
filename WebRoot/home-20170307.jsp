...
<link rel="stylesheet" type="text/css"
	href="http://localhost:8081/arcgisAPI317/library/3.17/3.17/dijit/themes/tundra/tundra.css" />
<link rel="stylesheet" type="text/css"
	href="http://localhost:8081/arcgisAPI317/library/3.17/3.17/esri/css/esri.css" />
<script type="text/javascript"
	src="http://localhost:8081/arcgisAPI317/library/3.17/3.17/init.js"></script>
...
<script type="text/javascript">
	...
	require([ "dojo/parser", "esri/map", "esri/dijit/Scalebar",
		...
		function(parser, Map, Scalebar, ArcGISDynamicMapServiceLayer,WMSLayer,
				...
				//var tilelayerurl="http://192.168.1.234/mapserver/tms/services/globaldem/MapServer";
                //var myTiledLayer = new TiledMapServiceLayer(tilelayerurl);
                //map.addLayer(myTiledLayer);
				
			//	var bsdmap=new fwVecLayer({url:"http://192.168.1.234/mapserver/tms/services/globaldem/MapServer/tile/"});//加载192.168.1.234底图服务
			//	map.addLayer(bsdmap);
			//	var bsdmap2=new fwVecLayer({url:"http://192.168.1.234/mapserver/tms/services/world100w/MapServer/tile/"});//加载192.168.1.234底图服务
			//	map.addLayer(bsdmap2);
                ...
                //var agsWmsUrl = "http://192.168.1.234/mapserver/wms/services/globaldem/WMSServer";
                var agsWmsUrl = "http://localhost:6080/arcgis/services/MyTDTMapService/MapServer/WMSServer";  
		        ...
		        var parcelsURL = "http://localhost:6080/arcgis/rest/services/BaseMap/MapServer";
		        map.addLayer(new ArcGISDynamicMapServiceLayer(parcelsURL,{ opacity: 0.95 }));
				function mapReady () {
			...
			*/
			...
			//document.getElementById('btnAdd').onclick=addGeoJsonLayer;		/***此处函数不能有()否则会在网页加载时自动运行***/
			//document.getElementById('btnAdd').addEventListener("click",addGeoJsonLayer);		/***此处函数不能有()否则会在网页加载时自动运行***/
			/* on(dom.byId("btnAdd"), "click",addGeoJsonLayer); */
			...
			$("#testsub").click(function(){
				...
				$.ajax({
					...
					success:function(data){						
						var obj= eval("("+data+")");
						/* var former=(typeof Terraformer !== 'undefined') ? Terraformer : null;						
						var json=new former.Primitive(obj);
						var arcgisJson=former.ArcGIS.convert(json);
						var maxDraw=10000;
						var drawCountTotal;
						// Limit size of data that can be drawn
						if (arcgisJson.length > maxDraw) {
						    //this._drawCountTotal = this._maxDraw;
							drawCountTotal = maxDraw;
							console.warn("GeoJsonLayer Warning: Large dataset detected. Only drawing the first " + maxDraw + " features!");
							alert("查询结果数据集超过限制，仅绘制前" + maxDraw + "条记录!");
						} else {
						    drawCountTotal = arcgisJson.length;
						}
						graLayer = new esri.layers.GraphicsLayer();
						// Add graphics to the layer with symbols, project if needed
						for (var i = 0; i < drawCountTotal; i++) {
						    feature = arcgisJson[i];
						    // Create graphic - magically sets the geometry type!
						    graphic = createGraphic(feature);
							// Add to layer
			...
			function setQuylist(data,layid){
				...
				if($('#userTable').bootstrapTable!=null)
				...
				for(var i=0;i<laygraphics.length;i++){
					...
					if(tmpgra.attributes.FID==graFID) {
						...
						var symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
									    new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,
									    new Color([255,0,0]), 2), new Color([98,194,204,0.5])
									  );
...
/* 				// Set the correct symbol based on type and render - NOTE: Only supports simple renderers
			...
			/* on(dom.byId("btntest"), "click",getquerylist); */
			function getquerylist(){
				alert('wqwqwed');
				//initisidebar();
				/* geoJsonLayer.clear(); */
				/* var Graphics=geoJsonLayer.graphics;
				var ids=map.layerIds.length;
				var layer= map.getLayer(map.layerIds[0]);
				map.removeLayer(graLayer); */
/* 				var Graphics=geoJsonLayer.graphics;
				var cnum=Graphics.length;
				cnum++; */
			}		
	...
	/* 		String[] strPNGfiles=null;
			...
			<%
			String[] strOverviewfiles=(String[])session.getAttribute("strOverviewfiles");
			String[] strOverviewExtents=(String[])session.getAttribute("strOverviewExtents");
			
		 	if(strOverviewfiles!=null)
		 	{
		 	%>
		 		imglayer=new MapImageLayer();
		 		map.addLayer(imglayer);
		 	<%
		 		System.out.println(strOverviewfiles.length+"   "+strOverviewExtents.length);
			 	for(int i=0;i<strOverviewfiles.length;i++)
				{
					String strFile=strOverviewfiles[i];
					String strExtent=strOverviewExtents[i];
			%>
					/* 注意此处一定要加双引号，否则无法识别 */
					var strfileurl="<%=strFile%>";
					var strfileextent=<%=strExtent%>;
			...
			<%			
				...
				session.setAttribute("strOverviewfiles",null);
				session.setAttribute("strOverviewExtents",null);
		 	}
			%>
...
<body class="tundra">
	<div data-dojo-type="dijit/layout/BorderContainer"
		...
		<div data-dojo-type="dijit/layout/ContentPane"
			...
			<div data-dojo-type="dijit/layout/ContentPane"
				...
				<div style="position:relative;bottom:10px;height:90%;font:normal 12px tahoma,arial,helvetica,sans-serif;">
					<form id="qureyform" action="servlet/Query" method="post">
						...
						<!-- <div style="padding:8px;padding-left:18px;">
							轨道号：<input name="orbitid" type="text" style="width:210px;" ><br />
						</div>
						<div style="padding:8px;padding-left:18px;">
							人工云量：
							<input name="cloud1" type="number" value=0 min=0 max=100 style="width:90px;" >
							至：<input name="cloud2" type="number" min=0 max=100  value=20 style="width:90px;"><br />
						</div>
						<div style="padding-left:18px;padding-bottom:15px;">
							  <div style="display:inline;float:left;">传感器标识：</div>
							  <div style="padding-left:8px;">
								    <div style="display:inline;float:left;padding-right:12px;">
								       <div  style="padding:2px;"><input type="checkbox" name="FWD" value="FWD"> 前视相机</div>
								       <div  style="padding:2px;"><input type="checkbox" name="BWD" value="BWD"> 后视相机</div>
								    </div>
								    <div style="display:inline;float:left;">
								      <div  style="padding:2px;"><input type="checkbox" name="NAD" value="NAD"> 正视相机</div>
								      <div  style="padding:2px;"><input type="checkbox" name="MUX" value="MUX"> 多光谱相机</div>
								    </div>
							  </div>						  
						</div><br/><br/>					
						<div style="padding:8px;padding-left:18px;">
							景 PATH：<input name="path1" type="number" style="width:90px;">至：<input name="path2" type="number" style="width:90px;"><br /> 
						</div>
						<div style="padding:8px;padding-left:18px;">
							景 ROW：<input name="row1" type="number" style="width:90px;"> 至：<input name="row2" type="number" style="width:90px;"><br /> 
						</div>
						<div style="padding:8px;padding-left:18px;">
							图像采集日期：<input id="date1" name="date1" class="Wdate" type="text" style="width:90px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'2012-01-01 00:00:00',maxDate:'#F{$dp.$D(\'date2\')||\'new Date()\'}'})"/>
	     					  至<input id="date2" name="date2" class="Wdate" type="text" style="width:90px;" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'date1\')}',maxDate:'%y-%M-%d'})"/> 
	  						</br>
						</div><br> -->
				...
				<!-- <div style="display:inline;float:left;">
				  <div style="display:inline;float:left;padding-left:52px;padding-right:20px;">
	...
	<div id="mapDiv" data-dojo-type="dijit/layout/ContentPane"
		...
		<div style="position:fixed;z-index:998;border:none;display:inline;float:left;background:white;font-size:12px;width:100%">
			...
			<button style="border:none;background:transparent;">导入shp</button>
	...
	<script type="text/javascript">
		...
		$("#btnquylistResize").click(function(){
			$('#resulist').bootstrapTable('resetView');
		});
		...
		$("#testmap").click(function(){		
			$.getJSON("data/temp.geojson",function(data){
				...
				var obj=new Array();
				for(i in features){
					var tmp= features[i].properties.F_SCENEDAT;
					tmp=tmp.substring(0,10);
					features[i].properties.F_SCENEDATE=tmp;
					obj.push(features[i].properties);
				}
				$('#resulist').bootstrapTable({
					method: 'get',
					...
					search: true,
					columns: [{field:"select",title:"全选",checkbox:true,width:20,align:"center",valign:"middle"},
					{field:"F_SCENEPATH",title:"PATH",align:"center",valign:"middle",sortable:"true"},
					{field:"F_SCENEROW",title:"ROW",align:"center",valign:"middle",sortable:"true"},
					{field:"F_ORBITID",title:"轨道号",align:"center",valign:"middle",sortable:"true"},
					{field:"F_OVERALLDATAQUALITY",title:"云量",align:"center",valign:"middle",sortable:"true"},
					{field:"F_SCENEDATE",title:"拍摄时间",align:"center",valign:"middle",sortable:"true"},
					],
					data:obj,
